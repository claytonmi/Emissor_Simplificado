...
begin
  ...
  StringGridList.ColCount := 6; // Define o número de colunas
  StringGridList.RowCount := 2;
  // Sempre inicia com 2 linhas (cabeçalho + 1 linha de dados)

  // Define os títulos do cabeçalho
  StringGridList.Cells[0, 0] := 'Código'; // Título da primeira coluna (Código)
  ...
  StringGridList.Cells[4, 0] := 'Total'; // Título da quinta coluna (Total)
  StringGridList.Cells[5, 0] := 'Data'; // Título da sexta coluna (Data)
  ...
  begin
    ...
    while not EOF do
    begin
      // Ajusta o número de linhas conforme necessário
    ...
    // Remove linhas extras (mantém pelo menos o cabeçalho)
    if i < StringGridList.RowCount then
      StringGridList.RowCount := Max(2, i);
    // Mantém 2 linhas (1 cabeçalho + 1 linha mínima)

    // Verifica se a grid está vazia para habilitar ou desabilitar o botão
    if i = 1 then
    begin
      BtEditarItem.Enabled := false; // Desabilita o botão se não houver itens
      BtExcluirItem.Enabled := false;
      BtFecharPedido.Enabled := false;
    end
    else
    begin
      BtEditarItem.Enabled := true; // Habilita o botão se houver itens
      BtExcluirItem.Enabled := true;
  ...
  // Se não houver itens, exibe uma linha vazia, mas mantém o cabeçalho
  if i = 1 then
  begin
    StringGridList.RowCount := 2;
    // Mantém a linha de cabeçalho e uma linha vazia
    StringGridList.Rows[1].Clear; // Limpa o conteúdo da linha vazia
  end;
end;
...
begin
  try
    ...
    // Se a data não for passada, o banco usará CURRENT_DATE como valor padrão
...
begin
  ...
  RodaPeVersion.Caption := 'Versão: 1.0';
...
begin
  ...
  begin
    ...
    StringGridList.RowCount := 1; // Limpa o grid (mantém a linha de cabeçalho)
    i := 1;
    while not Eof do
    begin
