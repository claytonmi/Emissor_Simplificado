...
begin
  Result := StringReplace(Email, '@', 'xws1d-', [rfReplaceAll]);
  Result := StringReplace(Result, '.', '02*1dfgh2', [rfReplaceAll]);
  Result := Result + '-x04c9p4';
end;
...
begin
  ...
  begin
    ShowMessage('Ativação concluída com sucesso!');
    ArmazenarChaveNoRegistro(Chave);
    ModalResult := mrOk;  // Fecha a tela de ativação
  end
  else
    ShowMessage('Chave inválida! Verifique os dados e tente novamente.');
end;
